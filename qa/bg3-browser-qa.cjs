...
const GWB_FILE = path.join(
  ROOT,
  "public/geology-map/data/bd_ibr_groundwater_bodies.geojson"
);

const OUT_DIR = path.join(
  ROOT,
  "qa",
  "bg3-browser-failures"
);
...
(async () => {
  ...
  if (bg3Profiles.length !== 41) {
    console.error(
      "Safety stop: expected 41 BG3 profiles, got",
      bg3Profiles.length
    );
    process.exit(2);
  }
  ...
  console.log("======================================");
  console.log("BG3 FULL BROWSER QA");
  console.log("PROFILES:", bg3Profiles.length);
  console.log("======================================");

  for (
    let index = 0;
    index < bg3Profiles.length;
    index++
  ) {
    const profile = bg3Profiles[index];
