-- ============================================================
-- SONDI.BG EXPERT - TIME-LIMITED UNLIMITED ACCESS
-- ============================================================

create table if not exists public.expert_time_access (
...
create index if not exists expert_time_access_user_dates_idx
  on public.expert_time_access(user_id, starts_at, expires_at);

alter table public.expert_time_access enable row level security;

drop policy if exists "users can read own expert time access"
  on public.expert_time_access;

create policy "users can read own expert time access"
  on public.expert_time_access
  for select
  to authenticated
...
begin
  ...
  -- ==========================================================
  -- TIME-LIMITED UNLIMITED ACCESS
  -- Does not consume free analyses or paid balance.
  -- ==========================================================

  if exists (
    select 1
    from public.expert_time_access
    where user_id = p_user_id
      and starts_at <= now()
      and expires_at > now()
  ) then
    ...
    values (
      ...
      0,
      v_analysis_id,
      'SONDI EXPERT timed unlimited access'
    );
    ...
    select
      ...
      'unlimited'::text,
      0,
      coalesce(v_wallet.free_analyses_remaining, 0),
