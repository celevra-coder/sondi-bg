...
export function getSpatialProfile(
  ...
  const blackSeaInvestigationData =
    readGeoJson(
      "bd_bs_hydrogeological_investigations.preview.geojson"
    );
  ...
  const blackSeaInvestigations: Feature[] =
    blackSeaInvestigationData.features || [];

  const investigationNearby =
    blackSeaInvestigations
      .map((feature: Feature) => ({
        distanceKm:
          featureDistance(feature, lat, lng),
        properties:
          feature.properties || {},
      }))
      .filter(
  ...
  const investigationsWithin5Km =
    investigationNearby.filter(
  ...
  return {
    nearestHydrogeologicalInvestigation:
      investigationNearby[0] ?? null,

    nearbyHydrogeologicalInvestigations:
      investigationsWithin5Km.slice(0, 10),

    hydrogeologicalInvestigationCounts: {
      km1: countWithin(
        blackSeaInvestigations, lat, lng, 1
      ),

      km3: countWithin(
        blackSeaInvestigations, lat, lng, 3
      ),

      km5: countWithin(
        blackSeaInvestigations, lat, lng, 5
      ),
    },
